...
public class ArenaCountdownEventHandler extends ArenaEventHandler {
  ...
  public ArenaCountdownEventHandler(
    TobnetArena arena,
    ArenaCountdownManager manager
  ) {
    super(arena, manager);
  ...
  public void gameStatusPreTransition(TobnetArenaStatusPreChangeEvent event) {
    if (getArena().equals(event.getArena())) {
      if (
        ArenaGameStatus.COUNTDOWN.equals(event.getPriorStatus()) &&
        ArenaGameStatus.IN_PROGRESS.equals(event.getNewStatus())
      ) {
        // The countdown has finished and the game has begun.
        countdownManager.completeCountdown();
      } else if (
        ArenaGameStatus.COUNTDOWN.equals(event.getPriorStatus()) &&
        !ArenaGameStatus.COUNTDOWN.equals(event.getNewStatus())
      ) {
  ...
  public void gameStatusPostTransition(TobnetArenaStatusPostChangeEvent event) {
    if (getArena().equals(event.getArena())) {
      if (
        ArenaGameStatus.WAITING.equals(event.getPriorStatus()) &&
        ArenaGameStatus.COUNTDOWN.equals(event.getNewStatus())
      ) {
        // Begin the countdown.
  ...
  public void countdownChatMessageSender(
    TobnetArenaCountdownDecrementedEvent event
  ) {
    if (
      getArena().equals(event.getArena()) &&
      countdownManager.getConfiguration().isDisplayChatIntervals()
    ) {
      ArenaChatManager chatManager = (ArenaChatManager) ArenaManagerUtils
        .getManagerOfType(getArena(), ArenaManagerType.CHAT)
        .get();

      if (
        (event.getNewValue() <= 15 &&
          event.getNewValue() > 5 &&
          event.getNewValue() % 5 == 0) ||
        (event.getNewValue() <= 5 && event.getNewValue() > 0)
      ) {
        // Send a message to all players in the arena.
        chatManager.sendMessageToAllPlayers(
          TobnetGamePlugin
            .getMessageController()
            .getMessage(
              MessageConstants.COUNTDOWN_GAME_BEGINS,
              TobnetTimeUtils.secondsToDuration(event.getNewValue())
            )
        );
      }
    }
