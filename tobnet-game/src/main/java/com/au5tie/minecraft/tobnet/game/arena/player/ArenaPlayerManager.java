...
public class ArenaPlayerManager extends ArenaManager {
  ...
  public final synchronized void joinGame(GamePlayer player) {
    ArenaGameManager gameManager = ArenaManagerUtils
      .getGameManager(getArena())
      .get();
    ...
    if (isArenaStatusJoinable()) {
      ...
      TobnetPlayerPreJoinEvent preJoinEvent = new TobnetPlayerPreJoinEvent(
        getArena(),
        player
      );
      ...
      if (!preJoinEvent.isCancelled()) {
        ...
        throw new ArenaStatusNotJoinableException(
          "Player pre-join event was cancelled."
        );
      }
    } else {
  ...
  private void performPlayerJoin(GamePlayer player) {
    ...
    TobnetEventPublisher.publishEvent(
      new TobnetPlayerPostJoinEvent(getArena(), player)
    );
  }
  ...
  private void performPlayerLeave(GamePlayer player) {
    ...
    TobnetEventPublisher.publishEvent(
      new TobnetPlayerLeaveEvent(getArena(), player)
    );
  ...
  private boolean isArenaStatusJoinable() {
    ...
    return (
      gameManager.isStatusEmpty() ||
      gameManager.isStatusWaiting() ||
      gameManager.isStatusCountdown()
    );
  }
