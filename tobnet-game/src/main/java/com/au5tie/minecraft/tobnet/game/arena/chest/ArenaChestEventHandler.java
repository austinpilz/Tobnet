...
public class ArenaChestEventHandler extends ArenaEventHandler {
  ...
  public void inventoryOpenEvent(InventoryOpenEvent event) {
    if (
      ...
      Optional<ArenaChest> chest = chestManager.getChestAtLocation(
   ...
   * @author au5tie.
   */
  private boolean isInventoryAlreadyOpenedByOthers(
    Inventory inventory,
    HumanEntity player
  ) {
    return (
  ...
  private void validateUnregisteredChestOpen(InventoryOpenEvent event) {
    if (!isChestRegistered(event.getInventory().getLocation())) {
      ...
      TobnetLogUtils.warn(
        ...
        " attempted to open a chest while playing" +
        " in arena " +
        getArena().getName() +
  ...
  private void validateMultipleChestOpens(
    ArenaChest chest,
    InventoryOpenEvent event
  ) {
    if (
      isInventoryAlreadyOpenedByOthers(event.getInventory(), event.getPlayer())
    ) {
