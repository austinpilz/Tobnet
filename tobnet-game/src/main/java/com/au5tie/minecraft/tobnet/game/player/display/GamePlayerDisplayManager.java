...
public class GamePlayerDisplayManager {
  ...
  public void destroyComponents() {
    displayComponents
      .values()
      .forEach(GamePlayerDisplayComponent::destroyComponent);
  ...
  public void registerComponent(GamePlayerDisplayComponent component) {
    if (
      displayComponents.containsKey(component.getName()) &&
      displayComponents.get(component.getName()) != component
    ) {
      // There's already another component with this name that is a different object. Reject registration.
      throw new DuplicatePlayerDisplayComponentException(
        "Another display component with the name " +
        component.getName() +
        " is already registered for the player " +
   ...
   * @author au5tie
   */
  private void performLocationPrioritization(
    GamePlayerDisplayComponentLocation location
  ) {
    // Determine which component is currently visible to the user.
    Optional<GamePlayerDisplayComponent> currentlyVisibleComponent =
      getCurrentlyVisibleComponentByLocation(location);

    // Find all components for this location which should be visible to the user.
    List<GamePlayerDisplayComponent> shouldBeVisibleComponents =
      getShouldBeVisibleComponentsByLocation(location);
    ...
    GamePlayerDisplayComponent highestPriorityComponent =
      getHighestPriorityComponent(shouldBeVisibleComponents);

    if (currentlyVisibleComponent.isPresent()) {
      // Check to see if the one visible is the highest priority.
      if (
        highestPriorityComponent != null &&
        highestPriorityComponent.isHigherPriorityThan(
          currentlyVisibleComponent.get()
        )
      ) {
   ...
   */
  private Optional<
    GamePlayerDisplayComponent
  > getCurrentlyVisibleComponentByLocation(
    GamePlayerDisplayComponentLocation location
  ) {
    return displayComponents
      ...
      .filter(component ->
        component.isVisible() && component.isShouldBeVisible()
      )
      .findFirst();
  }
  ...
  private List<
    GamePlayerDisplayComponent
  > getShouldBeVisibleComponentsByLocation(
    GamePlayerDisplayComponentLocation location
  ) {
    return displayComponents
      .values()
   ...
   * @author au5tie
   */
  private List<GamePlayerDisplayComponent> getComponentsByLocation(
    GamePlayerDisplayComponentLocation location
  ) {
    return displayComponents
      .values()
   ...
   * @author au5tie
   */
  private GamePlayerDisplayComponent getHighestPriorityComponent(
    List<GamePlayerDisplayComponent> components
  ) {
    if (CollectionUtils.isNotEmpty(components)) {
      // Sort the components by priority.
      components.sort(
        Comparator.comparing(GamePlayerDisplayComponent::getPriority)
      );
