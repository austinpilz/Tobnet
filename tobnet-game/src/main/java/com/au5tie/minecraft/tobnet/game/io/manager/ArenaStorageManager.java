...
public class ArenaStorageManager extends StorageManager {
  ...
  protected void prepareArenaTable() {
    ...
    try {
      ...
      statement.executeUpdate(
        "CREATE TABLE IF NOT EXISTS \"arena\" (\"Name\" VARCHAR PRIMARY KEY NOT NULL, \"Class\" VARCHAR NOT NULL, \"B1X\" DOUBLE, \"B1Y\" DOUBLE, \"B1Z\" DOUBLE, \"B2X\" DOUBLE, \"B2Y\" DOUBLE, \"B2Z\" DOUBLE, \"World\" VARCHAR)"
      );
  ...
  protected void loadData() {
    try {
      ...
      PreparedStatement preparedStatement = connection.prepareStatement(
        "SELECT `Name`, `Class`, `B1X`, `B1Y`, `B1Z`, `B2X`, `B2Y`, `B2Z`, `World` FROM `arena`"
      );
      ...
      TobnetLogUtils.error(
        "ArenaStorageManager >> loadArenas() >> Encountered an error while attempting to load arenas",
        exception
      );
    }
  }
  ...
  private void loadArena(ResultSet result) throws Exception {
    ...
    Location locationOne = new Location(
      ...
      result.getDouble("B1X"),
      result.getDouble("B1Y"),
      result.getDouble("B1Z")
    );
    arena.setBoundaryOne(locationOne);

    Location locationTwo = new Location(
      Bukkit.getWorld(result.getString("World")),
      result.getDouble("B2X"),
      result.getDouble("B2Y"),
      result.getDouble("B2Z")
    );
   ...
   * @author au5tie
   */
  private TobnetArena createArena(String arenaName, String className)
    throws Exception {
