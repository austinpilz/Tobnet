...
public class ArenaSetupSessionStepName extends SetupSessionStep {
  ...
  protected void displayPromptBody(SetupSessionStepInvocationContext context) {
    ...
    context
      .getPlayer()
      .sendMessage(
        "To configure the name of the arena, execute the command " +
        SetupSessionChatUtils.generateColoredChatSegment(
          "/" +
          TobnetGamePlugin
            .getSetupSessionController()
            .getCommandListener()
            .getFirstSupportedCommand()
            .orElse("UNKNOWN") +
          " setup arena [arenaName]",
          ChatColor.GREEN,
          ChatColor.WHITE,
          true
        )
  ...
  protected boolean invoke(SetupSessionStepInvocationContext context) {
    if (context.getCommandArguments().size() < 2) {
      ...
      TobnetChatUtils.sendPlayerMessage(
        context.getPlayer(),
        "The arena name was not provided in the command arguments."
      );
      return false;
    }
    ...
    if (
      TobnetGamePlugin
        .getArenaController()
        .getArenaByName(providedArenaName)
        .isPresent()
    ) {
      // An Arena with the provided name already exists.
      TobnetChatUtils.sendPlayerMessage(
        context.getPlayer(),
        "An arena with the name " +
        providedArenaName +
