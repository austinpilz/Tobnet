...
public class TobnetStorageController implements TobnetController {
    ...
    if (!TobnetGamePlugin.instance.getDataFolder().exists()) {
      try {
        ...
        TobnetLogUtils.error(
          "ExternalStorage >> createLocalDirectory() >> Encountered an error while attempting to create local directory",
          exception
        );
      }
    }
  ...
  public synchronized Connection getConnection() {
    ...
    try {
      ...
      TobnetLogUtils.error(
        "ExternalStorage >> getConnection() >> Encountered an error while attempting to verify if connection is already closed",
        exception
      );
    }
  ...
  private Connection createConnection() {
    try {
      ...
      Connection ret = DriverManager.getConnection(
        "jdbc:sqlite:" +
        new File(
          TobnetGamePlugin.instance.getDataFolder().getPath(),
          "db.sqlite"
        )
          .getPath()
      );
      ...
      TobnetLogUtils.error(
        "ExternalStorage >> createConnection() >> Encountered an error while attempting to create a new connection",
        exception
      );
      return null;
    }
  ...
  private synchronized void freeConnection() {
    ...
    if (connection != null) {
      try {
        ...
        TobnetLogUtils.error(
          "ExternalStorage >> freeConnection() >> Unable to free connection",
          exception
        );
      }
    }
  ...
  public void onStartup() {
    ...
    TobnetEventPublisher.publishEvent(
      new TobnetExternalStorageLoadCompleteEvent()
    );
  }
  ...
  private void loadData() {
    ...
    ArenaStorageManager arenaStorageManager = (ArenaStorageManager) getManager(
      StorageManagerType.ARENA
    );
