...
public class ArenaCountdownManager extends ArenaManager {
  ...
  public ArenaCountdownManager(
    TobnetArena arena,
    ArenaCountdownConfiguration configuration,
    ArenaCountdownSecondsProvider secondsProvider
  ) {
    super(arena);
  ...
  public void afterArenaPreparationComplete() {
    ...
    playerDisplayManager =
      (ArenaPlayerDisplayManager) ArenaManagerUtils
        .getManagerOfType(getArena(), ArenaManagerType.PLAYER_DISPLAY)
  ...
  private void decrementCountdown() {
    ...
    if (
      new TimeDifference(countdownLastUpdated, LocalDateTime.now())
        .getSecondsBetween() >=
      1
    ) {
      if (secondsLeft > 0) {
        ...
        TobnetEventPublisher.publishEvent(
          new TobnetArenaCountdownDecrementedEvent(
            getArena(),
            this,
            secondsLeft + 1,
            secondsLeft
  ...
  private void displayCountdownToUsers() {
    if (configuration.isDisplayCountdownUI()) {
      ...
      getPlayerDisplayManager()
        .getPlayerManager()
        .getPlayers()
        .forEach(player ->
          player
            .getDisplayManager()
            .requestComponentDisplay(getPlayerCountdownDisplayComponentName())
        );
      ...
      getPlayerDisplayManager()
        .getPlayerManager()
        .getPlayers()
        .forEach(this::updateUserCountdownDisplay);
    }
  }
  ...
  private void hideCountdownFromPlayers() {
    getPlayerDisplayManager()
      ...
      .forEach(player ->
        player
          .getDisplayManager()
          .requestComponentHide(getPlayerCountdownDisplayComponentName())
      );
  }
  ...
  private final void registerCountdownDisplayToUsers() {
    ...
    List<GamePlayer> players = getPlayerDisplayManager()
      .getPlayersMissingComponent(getPlayerCountdownDisplayComponentName());

    // Register a new display component for each one.
    players.forEach(this::registerCountdownDisplayToUser);
  }
   ...
   * @author au5tie
   */
  private void registerCountdownDisplayToUser(GamePlayer player) {
    // Create the countdown display component.
    GamePlayerDisplayComponent displayComponent = createUserCountdownDisplay(
      player
    );

    if (
      displayComponent != null &&
      displayComponent instanceof ArenaCountdownDisplayComponent
    ) {
      ...
      throw new TobnetEngineException(
        "The countdown display component provided to " +
        player +
        " must inherit from the ArenaCountdownDisplayComponent interface."
      );
    }
  }
   ...
   * @author au5tie
   */
  private GamePlayerDisplayComponent createUserCountdownDisplay(
    GamePlayer player
  ) {
    // Obtain the translated title.
    String title = TobnetGamePlugin
      .getMessageController()
      .getMessage(MessageConstants.COUNTDOWN_DISPLAY_TITLE);

    return new ArenaCountdownDisplayComponentBossBar(
      getPlayerCountdownDisplayComponentName(),
      2,
      player,
      title,
      BarColor.WHITE,
  ...
  private void updateUserCountdownDisplay(GamePlayer player) {
    Optional<GamePlayerDisplayComponent> component = player
      .getDisplayManager()
      .getComponent(getPlayerCountdownDisplayComponentName());

    if (
      component.isPresent() &&
      component.get() instanceof ArenaCountdownDisplayComponent
    ) {
      ArenaCountdownDisplayComponent displayComponent =
        (ArenaCountdownDisplayComponent) component.get();
  ...
  public List<String> getConsoleStatusLines(CommandSender sender) {
    return List.of(
      TobnetGamePlugin
        .getMessageController()
        .getMessage(MessageConstants.CONSOLE_COUNTDOWN) +
      ": " +
      getSecondsLeft() +
      " " +
      TobnetGamePlugin
        .getMessageController()
        .getMessage(MessageConstants.CONSOLE_SECONDS)
    );
  }
}
