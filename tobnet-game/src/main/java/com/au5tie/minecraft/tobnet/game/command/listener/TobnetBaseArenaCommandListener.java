...
public class TobnetBaseArenaCommandListener extends CommandListener {
  ...
  private void displayArenaList(CommandSender sender, String[] args) {
    if (!TobnetGamePlugin.getArenaController().getArenas().isEmpty()) {
      // Header.
      TobnetChatUtils.sendCommandSenderMessage(
        sender,
        TobnetGamePlugin
          .getMessageController()
          .getMessage(MessageConstants.ARENA_CONSOLE_HEADER)
      );
      ...
      TobnetChatUtils.sendCommandSenderMessage(
        ...
        TobnetGamePlugin
          .getMessageController()
          .getMessage(MessageConstants.ARENA_NONE_REGISTERED)
      );
    }
  }
  ...
  private String generateArenaDisplayLine(TobnetArena arena) {
    ArenaGameManager gameManager = (ArenaGameManager) ArenaManagerUtils
  ...
  private void displayArenaInformation(CommandSender sender, String arenaName) {
    ...
    if (arena.isEmpty()) {
      ...
      TobnetChatUtils.sendCommandSenderMessage(
        ...
        TobnetGamePlugin
          .getMessageController()
          .getMessage(MessageConstants.ARENA_NO_EXIST)
      );
    }
