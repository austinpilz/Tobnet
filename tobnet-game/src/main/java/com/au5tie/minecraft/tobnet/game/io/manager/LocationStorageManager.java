...
public class LocationStorageManager extends StorageManager {
  ...
  public LocationStorageManager(
    TobnetStorageController tobnetStorageController
  ) {
  ...
  protected void prepareLocationTable() {
    ...
    try {
      ...
      statement.executeUpdate(
        "CREATE TABLE IF NOT EXISTS \"location\" (\"Name\" VARCHAR PRIMARY KEY NOT NULL, \"Class\" VARCHAR NOT NULL, \"Arena\" VARCHAR NOT NULL, \"Type\" VARCHAR NOT NULL, \"X\" DOUBLE, \"Y\" DOUBLE, \"Z\" DOUBLE, \"Pitch\" FLOAT, \"Yaw\" FLOAT, \"World\" VARCHAR, \"Metadata\" TEXT, FOREIGN KEY(\"Arena\") REFERENCES \"Arena\"(\"Name\"))"
      );
      ...
      TobnetLogUtils.error(
        "LocationStorageManager >> prepareLocationTable() >> ",
        exception
      );
    }
  }
  ...
  protected void loadData() {
    if (
      TobnetGamePlugin.getArenaController() != null &&
      CollectionUtils.isNotEmpty(
        TobnetGamePlugin.getArenaController().getArenas()
      )
    ) {
      ...
      TobnetGamePlugin
        .getArenaController()
        .getArenas()
        .forEach(this::loadLocationsForArena);
    }
  }
  ...
  private void loadLocationsForArena(TobnetArena arena) {
    try {
      ...
      PreparedStatement preparedStatement = connection.prepareStatement(
        "SELECT `Name`, `Class`, `Arena`, `Type`, `X`, `Y`, `Z`, `World`, `Yaw`, `Pitch`, `Metadata` FROM `location` WHERE `Arena` = ?"
      );
      ...
      if (count > 0) {
        TobnetLogUtils.info(
          "Loaded " + count + " location(s) for " + arena.getName() + "."
        );
      }
    } catch (Exception exception) {
      // We ran into some issue while attempting to load locations for this arena.
      TobnetLogUtils.error(
        "LocationStorageManager >> loadLocations() >> Encountered an error while attempting to load locations for arena " +
        arena.getName(),
        exception
      );
    }
  ...
  private void loadLocation(ResultSet result) {
    try {
      ...
      Optional<TobnetArena> arena = TobnetGamePlugin
        .getArenaController()
        .getArenaByName(result.getString(LOCATION_COLUMN_ARENA));

      if (!arena.isPresent()) {
        // The arena this location belongs to is not loaded.
        TobnetLogUtils.warn(
          "LocationStorageManager >> loadLocation() >> Found location for non-existent Arena " +
          result.getString(LOCATION_COLUMN_ARENA)
        );
        return;
      }
      ...
      Location serverLocation = new Location(
        Bukkit.getWorld(result.getString(LOCATION_COLUMN_WORLD)),
        result.getDouble(LOCATION_COLUMN_X),
        result.getDouble(LOCATION_COLUMN_Y),
        result.getDouble(LOCATION_COLUMN_Z),
        result.getFloat(LOCATION_COLUMN_YAQ),
        result.getFloat(LOCATION_COLUMN_PITCH)
      );

      // Create the location via reflection.
      ArenaLocation arenaLocation = createLocation(
        result.getString(LOCATION_COLUMN_CLASS),
        arena.get(),
        result.getString(LOCATION_COLUMN_NAME),
        result.getString(LOCATION_COLUMN_TYPE),
        serverLocation
      );

      // Metadata conversion.
      Map<String, String> metadata = metadataFromJson(
        result.getString(LOCATION_COLUMN_METADATA)
      );
      metadata.forEach((key, value) ->
        arenaLocation.addMetadataElement(key, value)
      );

      // Register the finalized location with the arena's location manager.
      ArenaLocationManager locationManager =
        (ArenaLocationManager) ArenaManagerUtils
          .getManagerOfType(arena.get(), ArenaManagerType.LOCATION)
          .orElseThrow(TobnetEngineMissingManagerException::new);
      locationManager.loadLocation(arenaLocation);
    } catch (Exception exception) {
      // We encountered some error while loading this location.
      TobnetLogUtils.error(
        "LocationStorageManager >> loadLocation() >> There was an error attempting to load a location {" +
        result.toString() +
        "}",
        exception
      );
   ...
   * @author au5tie
   */
  private Map<String, String> metadataFromJson(String rawMetadata)
    throws Exception {
   ...
   * @author au5tie
   */
  private ArenaLocation createLocation(
    String className,
    TobnetArena arena,
    String name,
    String type,
