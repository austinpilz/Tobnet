...
public class TobnetSetupSessionController implements TobnetController {
   ...
   * @author au5tie
   */
  public void registerSessionType(
    Class<? extends SetupSession> sessionClass,
    String commandLevelType
  ) {
  ...
  public Optional<Class> getSupportedSessionClass(String sessionType) {
    return Optional.ofNullable(
      supportedSessionTypes.get(sessionType.toUpperCase())
    );
  }
   ...
   * @author au5tie
   */
  public SetupSession requestNewSession(
    SetupSessionStepInvocationContext context
  ) {
    if (
      !doesUserHaveExistingSession(context.getPlayer().getUniqueId().toString())
    ) {
  ...
  private SetupSession beginNewSession(String sessionType, Player player) {
    ...
    SetupSessionStepInvocationContext context =
      SetupSessionStepInvocationContext.builder().player(player).build();
  ...
  private SetupSession prepareNewSession(String sessionType, Player player) {
    ...
    try {
      ...
      Constructor sessionConstructor = sessionClass
        .orElseThrow(SetupSessionTypeNotSupportedException::new)
      ...
      throw new TobnetEngineException(
        "Encountered an error while attempting to create new setup session",
        exception
      );
    }
  }
  ...
  public final Optional<String> getUserExistingSessionType(String playerUuid) {
    ...
    if (userSession.isPresent()) {
      Optional<Map.Entry<String, Class>> sessionClassEntry =
        supportedSessionTypes
          .entrySet()
          .stream()
          .filter(entry -> entry.getValue().equals(userSession.get().getClass())
          )
          .findAny();
